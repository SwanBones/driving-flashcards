...
const easterEggs = {
  ...
  bwavtjysixrceiffuuelzjqeicdyrkgntn: "100% Félicitations!",
};

export default function Home() {
  const questions = useQuestions((s) => s.questions);
  const questionGroups = useQuestions((s) => s.questionGroups);
  const checkQuestions = useQuestions((s) => s.checkQuestions);
      ...
      {!hideGrid && (
        <div className="max-w-6xl flex flex-col items-center gap-8 ">
          ...
          <div className="bg-zinc-100 rounded-xl p-2 sm:p-10 flex flex-col items-center">
            <div>
              {showUnfinished && (
                <p className="text-zinc-500 mb-5 mt-5 text-center sm:-mt-5 flex gap-2 items-center">
                  <span>
                    <InfoCircleOutlined />
                  </span>
                  Questions non cochées surlignées
                </p>
              )}
            </div>
            <QuestionGrid
